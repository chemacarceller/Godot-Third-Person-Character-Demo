...
func _enter_tree() -> void : MyLogger.info("FRAME : " + str(Engine.get_process_frames()) + " : " + str(get_path()) + " Instantiated ... ", 'world_environment.gd',13,true)
		...
		if GameInstance.graphicsProfile == ConfigRender.Profile.DEFAULT :
			...
			match GameInstance.graphicsProfile :
				ConfigRender.Profile.LOW :
					...
					EventBus.emit.call_deferred(_ready, EventBus.EVENT.GraphicProfile_Changed, "LOW")
				ConfigRender.Profile.MEDIUM :
					ConfigRender.apply_graphics_profile(ConfigRender.Profile.MEDIUM, null, environment)
					EventBus.emit.call_deferred(_ready, EventBus.EVENT.GraphicProfile_Changed, "MEDIUM")
				ConfigRender.Profile.HIGH :
					ConfigRender.apply_graphics_profile(ConfigRender.Profile.HIGH, null, environment)
					EventBus.emit.call_deferred(_ready, EventBus.EVENT.GraphicProfile_Changed, "HIGH")
				ConfigRender.Profile.ULTRA :
					ConfigRender.apply_graphics_profile(ConfigRender.Profile.ULTRA, null, environment)
					EventBus.emit.call_deferred(_ready, EventBus.EVENT.GraphicProfile_Changed, "ULTRA")

		MyLogger.info("FRAME : " + str(Engine.get_process_frames()) + " : " + str(get_path()) + " Ready ... ", 'world_environment.gd',88,true)
